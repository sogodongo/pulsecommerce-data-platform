...
async def verify_api_key(api_key: str = Security(API_KEY_HEADER)) -> str:
    ...
    if api_key not in _VALID_KEYS:
        # refresh once and retry to handle key rotation
        _VALID_KEYS = _get_valid_api_keys()
        if api_key not in _VALID_KEYS:
            raise HTTPException(
...
async def daily_metrics(
    ...
    try:
        ...
        # Redshift Data API is async — poll until the statement settles
        for _ in range(30):
...
def _parse_redshift_result(result: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for record in result.get("Records", []):
        ...
        for col, cell in zip(columns, record):
            # each cell is a single-key dict like {"stringValue": ...} or {"longValue": ...}
